%% LATEST VERSION
% Input file: '[YIDATA]/mig5_pikqtime_1018_10pctB.raw'
...
system('rm M10_AKM_10pctB_czind.log')
diary('M10_AKM_10pctB_czind.log')
clear

disp('AKM estimation - model 10 - 10 pct B')
...
%% LOAD DATA
s=[yidata '/mig5_pikqtime_1018_10pctB.raw'];
tic
data=importdata(s);
% data=data(1:100000000,1:end); %subsetting for troubleshooting
...
age=age(sel); naics2d=naics2d(sel); naics2d_old=naics2d_old(sel); N=length(y); 
cz=cz(sel); cz_old=cz_old(sel);
...
% s=[datadir '/testing_estfile' int2str(i) '.txt'];
