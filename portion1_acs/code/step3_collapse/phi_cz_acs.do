*program phi_cz_acs to estimate CZ diffs and worker quality by CZ
*uses wage model from industry paper

*     step1: clean up industry codes 
*     step2: fit wage model get ind effects and xb by CZ, with and without extra controls
*     step3: collapse to CZ and output

cap log close
log using phi_cz_acs.log, replace
...
collapse (mean) logwage female educ cz_effects_m1 cz_effects_m2 cz_effects_m3  skill_m1 skill_m2 skill_m3  (sum) count=iw wcount=c [pw=pweight], by(cz)

save ${scratch}/czeffects, replace

sum
sum [w=wcount]
reg logwage cz_effects_m1 skill_m1 [w=wcount]
reg logwage cz_effects_m1  [w=wcount]

reg logwage cz_effects_m2 skill_m2 [w=wcount]
reg logwage cz_effects_m2  [w=wcount]

reg logwage cz_effects_m3 skill_m3 [w=wcount]
reg logwage cz_effects_m3  [w=wcount]
...
reg cz_effects_m3 logwage      [w=wcount]

reg skill_m2 logwage            [w=wcount]
reg cz_effects_m2 logwage      [w=wcount]

reg skill_m1 logwage            [w=wcount]
reg cz_effects_m1 logwage      [w=wcount]


list cz count wcount logwage educ female cz_effects_m3 skill_m3 
